<x-layouts.app-layout>
    <div class="container">
        <div class="flex justify-center">
            <form class="max-w-[520px] w-[100%] border p-[30px] border-dashed border-lime-400 mb-6" id="editInventoryForm"
                  action="{{ route('inventory.update-confirm') }}" method="POST">
                @method('POST')
                @csrf
                @if (session('editInventoryData')['deleteImageIds'] ?? false)
                    @foreach (session('editInventoryData')['deleteImageIds'] as $id)
                        <input name="deleteImageIds[]" type="hidden" value="{{ $id }}">
                    @endforeach
                @endif
                ...
                <div class="mb-6">
                    ...
                    <select class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-lime-500 focus:border-lime-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-lime-500 dark:focus:border-lime-500"
                        ...
                        @foreach ($categories as $category)
                            <option value="{{ $category->id }}" @selected(old('category_id', session('editInventoryData')['category_id'] ?? $store->category_id) == $category->id)>{{ $category->name }}
                            </option>
                        @endforeach
                    </select>
                    @error('category_id')
                ...
                <div class="mb-6">
                    ...
                    <textarea class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-lime-500 focus:border-lime-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-lime-500 dark:focus:border-lime-500"
                              id="message" name="description" rows="4" placeholder="Write your thoughts here...">{{ old('description', session('editInventoryData')['description'] ?? $store->description) }}</textarea>
                </div>
                    ...
                    <div class="grid grid-cols-3 gap-3 mb-6">
                        ...
                        @foreach ($store->images as $image)
                            <div class="block relative h-[100px] p-2 cursor-pointer bg-white border border-gray-200 rounded-lg shadow-md hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700"
                                ...
                                <img class="w-[100%] h-[100%]" src="{{ Storage::url($image->url) }}" alt="image"
                                     loading="lazy">
                            </div>
                        @endforeach
                        @foreach (session('base64Images', []) as $image)
                            @if (!in_array($image['id'], session('editInventoryData')['deleteImageIds'] ?? []))
                                <div class="block relative h-[100px] p-2 cursor-pointer bg-white border border-gray-200 rounded-lg shadow-md hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700"
                                     x-ref="image-{{ $image['id'] }}">
                        ...
                        <template x-for="image in serverImages">
                            <div
                                 class="block relative h-[100px] p-2 cursor-pointer bg-white border border-gray-200 rounded-lg shadow-md hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
                                <div
                                     class="hover:bg-slate-200 group/item absolute top-0 left-0 w-[100%] h-[100%] z-10 flex justify-center items-center">
                                    <svg class="w-6 h-6 text-red-500 invisible group-hover/item:visible"
                                         @click="removeImage(`${image.id}`)" xmlns="http://www.w3.org/2000/svg"
    ...
    <x-slot:scripts>
        <script>
            const serverData = {
                ...
                oldImages: @json($store->images->pluck('id')),
                deleteOldImageIds: @json(session('editInventoryData')['deleteOldImageIds'] ?? [])
            }
        </script>
